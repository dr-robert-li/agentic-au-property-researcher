...
Output: New validation.py module (created from scratch) with Pydantic schemas, wired into discovery and research flows.
</objective>
...
<task type="auto">
  ...
  <action>
Create NEW file `src/research/validation.py` (this file does not exist yet) with Pydantic v2 validation schemas for both discovery and research API responses.
...
```
  ...
  <done>
`src/research/validation.py` exists (new file) with `DiscoverySuburbResponse`, `ResearchSuburbResponse` schemas, `coerce_numeric` helper, `validate_discovery_response()` and `validate_research_response()` functions. String prices coerced to numbers. Missing required fields produce structured field-level warnings. Missing optional fields produce warnings but do not block processing.
  </done>
</task>
...
# Verify validation is actually called (not just referenced as a string)
# Parse the AST to find function calls to validate_* before cache.put
import inspect

# Check discovery: validate_discovery_response must be called
disc_source = inspect.getsource(discover_suburbs)
disc_tree = ast.parse(textwrap.dedent(disc_source))
disc_calls = [
    node.func.id for node in ast.walk(disc_tree)
    if isinstance(node, ast.Call) and isinstance(node.func, ast.Name)
]
assert 'validate_discovery_response' in disc_calls, (
    'discover_suburbs does not call validate_discovery_response()'
)
print('OK: discovery calls validate_discovery_response()')

# Check research: validate_research_response must be called
res_source = inspect.getsource(research_suburb)
res_tree = ast.parse(textwrap.dedent(res_source))
res_calls = [
    node.func.id for node in ast.walk(res_tree)
    if isinstance(node, ast.Call) and isinstance(node.func, ast.Name)
]
assert 'validate_research_response' in res_calls, (
    'research_suburb does not call validate_research_response()'
)
print('OK: research calls validate_research_response()')
...
5. Discovery pipeline calls validate_discovery_response before cache.put (verified via AST)
6. Research pipeline calls validate_research_response before cache.put (verified via AST)
...
- Discovery and research pipelines both wire through validation (verified by AST call analysis, not string matching)
