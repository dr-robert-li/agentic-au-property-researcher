...
must_haves:
  truths:
    ...
    - "Background research progress is available via a polling endpoint that returns queued messages without blocking"
  artifacts:
...
NOTE: This plan provides the BACKEND infrastructure for progress reporting (thread-safe queues and a polling endpoint). The browser UI consumption of this endpoint (SSE streaming, live progress bars) is handled in Phase 4 (Plan 04-01). This plan's `/api/progress/{run_id}` endpoint is the contract point between Phase 2 and Phase 4.
</objective>
...
9. **Add progress endpoint** (new):
   ...
   This endpoint is the backend contract for Phase 4's SSE streaming. Phase 4 (Plan 04-01) will build the browser-side polling/SSE client that consumes this endpoint. This plan only provides the server-side infrastructure.
...
Run: `python -c "from ui.web.server import active_runs_lock, completed_runs_lock, progress_queues; import threading, queue; assert isinstance(active_runs_lock, type(threading.Lock())); assert isinstance(completed_runs_lock, type(threading.Lock())); assert isinstance(progress_queues, dict); print('OK: locks and progress_queues verified')"` from `src/` directory.
  ...
  <done>
All `active_runs` and `completed_runs` access is wrapped with respective locks. Progress callbacks use `queue.Queue` instead of direct dict mutation. New `/api/progress/{run_id}` endpoint drains progress queue (backend infrastructure for Phase 4's SSE client). `copy.deepcopy()` used for all cross-thread state reads.
  </done>
</task>
...
- `/api/progress/{run_id}` endpoint provides backend contract for Phase 4 SSE streaming
