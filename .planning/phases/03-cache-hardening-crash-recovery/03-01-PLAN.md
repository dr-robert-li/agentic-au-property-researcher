...
must_haves:
  truths:
    ...
    - "Cache stats report shows how many orphaned files were removed on last startup (verifiable via stats() output)"
...
6. **Add _cleanup_orphans() method** called at end of __init__():
   ...
   - Store the count as `self._last_orphan_cleanup_count` for reporting via stats()
...
10. **Update stats() method**:
    - Add `max_size_bytes` to returned dict (from config.max_size_bytes)
    - Add `orphans_cleaned_last_startup` to returned dict (from self._last_orphan_cleanup_count)
    - Use entry.size_bytes instead of stat() calls for total_size (faster, consistent), but fall back to stat() if size_bytes is 0 (backward compat with old entries)
...
with tempfile.TemporaryDirectory() as td:
    ...
    # Verify stats includes max_size and orphan count
    stats = cache.stats()
    assert 'max_size_bytes' in stats
    assert 'orphans_cleaned_last_startup' in stats
    print('stats OK')
...
"`
  ...
  <done>
Cache writes use atomic temp-file + fsync + rename pattern. Cache index has backup that is created on every save. Index load falls back to backup on corruption. Orphan files are cleaned on startup and the count is exposed via stats(). LRU eviction enforces size limit. CacheEntry tracks size_bytes and last_accessed. All existing public API preserved.
  </done>
</task>
...
3. Orphan cleanup: `_cleanup_orphans()` runs in `__init__()`, deletes unindexed files, stores count for stats()
...
- _cleanup_orphans() removes files not in index on startup and stores count in self._last_orphan_cleanup_count
- stats() returns orphans_cleaned_last_startup and max_size_bytes
